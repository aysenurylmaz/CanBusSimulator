...
void MainWindow::generateCanFrame() {
    ...
    // Her 1 saniyede (10 tick) bir cache'i temizle ki Web UI sonradan açılırsa senkronize olabilsin.
    frameTickCounter++;
...
void MainWindow::physicsLoop() {
    ...
    // Daha dinamik motor sıcalığı fiziği
    if (currentSpeed > 0 && motorTemp < 105.0) {
        // Hız arttıkça daha hızlı ısınır (hız 100 ise 100ms'de +0.1 derece -> saniyede +1 derece)
        motorTemp += (currentSpeed / 1000.0); 
        inverterTemp += (currentSpeed / 1200.0);
    } else if (currentSpeed == 0 && motorTemp > 25.0 && !isHvacOn) {
        // Araç durduğunda yavaşça ortam sıcaklığına (25) soğur
        motorTemp -= 0.05; 
        inverterTemp -= 0.05;
    } 

    if (isHvacOn) {
        // Klima açıksa aktif soğutma (ortam sıcaklığı 20'ye kadar)
